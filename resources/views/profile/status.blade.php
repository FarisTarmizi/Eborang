        ...
        @if($brg->status === 'X')
            <ul class="step-wizard-list">
                <li class="step-wizard-item @if($brg->status==='P') current-item @endif">
                    ...
                    <span class="progress-label">Permohonan<br>Peralatan</span>
                </li>
                <li class="step-wizard-item @if($brg->status==='P') current-item @endif">
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            @if($brg->status === 'P')
                                @if(is_null($brg->review))
                                    <td class="text-warning">KB : <br> Sedang Menunggu Pengesahan KB Anda</td>
                                @else
                                    <td class="text-warning">KB BTM : <br> Sedang Menunggu Pengesahan KB BTM</td>
                                @endif
                            @elseif($brg->status === 'M')
                                <td class="text-success">
                                    KB BTM : <br> Telah disahkan - permohonan diserahkan kepada staff btm
                                </td>
                            @elseif($brg->status === 'R')
                                @if((!is_null($brg->gear_id)))
                                    <td class="text-info">
                    ...
                    {{--@if($brg->status === 'P')
                        @if(is_null($brg->review))
                            <span class="text-warning">KB : Sedang Menunggu Pengesahan KB Anda</span>
                        @else
                            <span class="text-warning">KB BTM : Sedang Menunggu Pengesahan KB BTM</span>
                        @endif
                    @elseif($brg->status === 'M')
                    ...
                    @elseif($brg->status === 'R')
                        ...
                        STAFF BTM : Peralatan telah sedia untuk diambil. <br>
                        Sila hadir  Bahagian Teknologi Maklumat bagi pengambilan peralatan
                    </span>
                        @else
                            <span class="text-info">STAFF BTM : proses pencarian peralatan</span>
                        @endif
                    @elseif($brg->status === 'B')
                        <span class="text-info">Sedang fasa pinjaman peralatan ICT</span>
                        <br> <br>
                        <b>Tarikh pulangan peralatan : </b>
                        <br>
                        @if(!($current>$end))
                    ...
                    baki lagi
                    <b>
                       {{$current->diff($end)->d + 1 .' hari'}}
                    </b>
                    <br>
                    (1 hari ditambah bagi proses pulangan)
                </span>
                        @else
                            @if($current->diff($end)->d === 0)
                                <div class="text-warning">
                                ...
                                <span class="badge badge-danger animation__wobble">
                       Hari ini !!!
                </span>
                            @else
                                <div class="text-danger">
                    ...
                    @elseif($brg->status === 'X')
                      ...
                      <b>  Permohonan anda telah ditolak. <br>Ulasan {{$mssg[0]}}: <br> </b>{{$mssg[1]}}
                        <p class="text-info">Sila tekan butang 'maklum' di bawah untuk membolehkan melakukan permohonan baru </p>
                    </span>
                        <a href="#" class="btn btn-info"
                           style="width: 150px" data-toggle="modal" data-target="#maklum">
                            MAKLUM
                        ...
                        <div class="modal fade" id="maklum" data-backdrop="static">
                            <div class="modal-dialog">
                                <div class="modal-content bg-info">
                                    ...
                                    <form action="{{route('borrower.remove',['id'=>$brg->id])}}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <div class="modal-body">
                                            Anda sekiranya mempunyai sebarang kemuskilan,sila hubungi bahagian IT.
                                        </div>
                                        <div class="modal-footer">
                                            <input type="submit" class="btn btn-outline-dark" style="width: 100px;"
                                                   value="Yes">
                                        </div>
                                    </form>
                                </div>
                    ...
                    @else
                        ...
                        <p class="text-info">
                            Sila tekan butang 'sah' di bawah bagi melakukan permohonan baru di masa akan datang
                        </p>
